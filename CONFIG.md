...
- Eureka instance-id: `서비스명:실제포트` (예: `premium-plans-service:51234`) — 기동 로그와 Eureka 대시보드에서 확인
...
| premium-plans-service | `premium_plans_db` |

로컬 JDBC URL 예시 (호스트·포트·스키마는 `.env.example` 참고):

```
jdbc:mysql://<host>:<port>/premium_plans_db?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=Asia/Seoul&characterEncoding=UTF-8
```
...
# 2. .env 설정 후 Premium Plans Service 실행
cd ../premium-plans-service && ./gradlew bootRun
```

- `bootRun` 은 프로젝트 루트의 `.env` 를 자동 로드합니다.
- IDE에서 실행할 때는 **Run Configuration** `PremiumPlansServiceApplication` 을 사용하거나, `PremiumPlansServiceApplication.java` 에서 main 실행 시 Working directory 가 프로젝트 루트인지 확인하세요.
- `application-local.yml` 이 `optional:file:.env[.properties]` 를 import 하므로 IDE main 실행 시에도 `.env` 가 로드됩니다.

Eureka Dashboard (`http://localhost:8761`) 에서 `PREMIUM-PLANS-SERVICE` 등록을 확인합니다.

## 필수 환경변수 (local)

| 변수 | 설명 |
|------|------|
| `SPRING_DATASOURCE_URL` | MySQL JDBC URL (`premium_plans_db` 스키마) |
...
```
com.sparta.premium_plans_service/
├── domain/model/              # 도메인 모델
├── application/
│   ├── service/               # Use Case 구현
│   ├── exception/
